...
import { optionIdFor, parseProjects, parseRepos, parseStatusField } from '../src/providers/github-discovery.js';
...
describe('github provider — status ↔ column mapping', () => {
  ...
  it('degrades gracefully when To Review collapses onto In Progress (first match wins)', () => {
    const collapsed = { todo: 'Todo', inProgress: 'In Progress', toReview: 'In Progress', done: 'Done' };
...
describe('github provider — command construction', () => {
  it('builds the issue-create args with a body file and repo', () => {
    ...
    expect(args).toEqual(['gh', 'issue', 'create', '--title', 'T', '--body-file', '/tmp/body.md', '--repo', 'acme/app']);
  });

  it('builds the item-add args (json for the item id)', () => {
    const args = itemAddArgs('acme', 5, 'https://github.com/acme/app/issues/7');
    expect(args).toEqual([
      'gh', 'project', 'item-add', '5', '--owner', 'acme',
      '--url', 'https://github.com/acme/app/issues/7', '--format', 'json',
    ]);
  });
  ...
  it('builds the item-edit args to move a card', () => {
    ...
    expect(args).toEqual([
      ...
      '--id', 'PVTI_2', '--project-id', 'PVT_1', '--field-id', 'PVTSSF_3', '--single-select-option-id', 'opt_4',
    ]);
  });
});
...
describe('github provider — item-list parsing', () => {
  const json = JSON.stringify({
    items: [
      { id: 'PVTI_a', status: 'Todo', content: { type: 'Issue', number: 7, body: '<!-- kodi:ticket {} -->' } },
  ...
  it('keeps only issue items and captures status/body (body absent → undefined)', () => {
    ...
    expect(items[0]).toMatchObject({ itemId: 'PVTI_a', statusName: 'Todo', body: '<!-- kodi:ticket {} -->' });
...
describe('github discovery — parsing', () => {
  it('parses the project list', () => {
    const projects = parseProjects(JSON.stringify({ projects: [{ number: 5, title: 'Roadmap', id: 'PVT_x' }] }));
    ...
    const field = parseStatusField(
      JSON.stringify({
        fields: [
          ...
          { id: 'PVTSSF_status', name: 'Status', options: [{ id: 'o1', name: 'Todo' }, { id: 'o2', name: 'Done' }] },
        ],
      }),
    );
  ...
  it('returns null when there is no Status field', () => {
    expect(parseStatusField(JSON.stringify({ fields: [{ id: 'x', name: 'Title', type: 'text' }] }))).toBeNull();
  });

  it('parses the repo list into owner/repo names', () => {
    expect(parseRepos(JSON.stringify([{ nameWithOwner: 'acme/app' }, { nameWithOwner: 'acme/api' }]))).toEqual([
      'acme/app',
      'acme/api',
    ]);
  });
